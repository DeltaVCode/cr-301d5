...
SELECT books.id, bookshelf, bookshelves.*
FROM books
INNER JOIN bookshelves
  ON books.bookshelf = bookshelves.name;
...
ALTER TABLE books ADD CONSTRAINT fk_bookshelves
FOREIGN KEY (bookshelf_id) REFERENCES bookshelves(id);

<!-- Query 7: Get all books + their bookshelf -->
SELECT books.id, title, name AS bookshelf
FROM books
INNER JOIN bookshelves
